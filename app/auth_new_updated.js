...
const ngssNONCPLdata_socials = require('./no-cpl-socials.js');
...
let customFiles1 = ["661_7490_ese_ee3_erthdifrgnlclmts.html"];
...
justParams = async(widgetSrc,json,cssparamValue,file)=>{
    return new Promise((resolve,reject)=>{
        if(appManifesto.gradeLevel == 'kindergarten'){
			...
			const startPosjson_one = (appManifesto.projectType == 'socials' ? '"../../widget_data/config/'+json.jsonPath+'"></param>' : '"../../../widget_data/config/'+json.jsonPath+'"></param>');
			let endPosjson = null;
			if(appManifesto.process == 'CPL'){
				endPosjson = '"../../../../widget_data/config/'+json.jsonPath+'" /> <param name="theme" value="'+cssparamValue+'" />';
			} else {
				endPosjson = '"../../../../widget_data/config/'+json.jsonPath+'" />';
			}
            ...
            const startPosjson_two = (appManifesto.projectType == 'socials' ? '"../../widget_data/config/'+json.jsonPath+'" />' : '"../../../widget_data/config/'+json.jsonPath+'" />');
            ...
            const startPosjson_three = (appManifesto.projectType == 'socials' ? '"../../widget_data/config/'+json.jsonPath+'"/>' : '"../../../widget_data/config/'+json.jsonPath+'"/>');
...
justObjects = async(widgetSrc,htmlPath,file)=>{
    return new Promise(async(resolve,reject)=>{
		if(appManifesto.process == 'CPL'){
			...
			let widgetSrc = (appManifesto.projectType == 'socials' ? ngssNONCPLdata_socials['matchingProps'][appManifesto.widgetType] : ngssNONCPLdata['matchingProps'][appManifesto.widgetType]);
...
writeJSONS = async(jsonData,json,file)=>{
	console.log(jsonData,"   jsonData");
	return new Promise(async(resolve,reject)=>{
		if(jsonData != null){
			console.log("in  ");
			fs.writeFile(checkoutDir+jsonDirectory+'/'+json.jsonPath,jsonData , 'utf-8',function (err) {
				let jsonObj = json;
				if (err) {
    ...
    let jsonValue = new Promise(async (resolve,reject) => {
        ...
        if(processType == 'audioOnce') {
            if(appManifesto.process == 'CPL'){
                ...
                /* method to write files */
				let writtenJSON = await writeJSONS(null,json,file);
				console.log(writtenJSON, "   writtenJSON");
				if(writtenJSON){
					const just_params = await justParams(widgetSrc,json,cssparamValue,file);
					htmlPath = null;
...
async function updatewidgets(activities,file,param_type){	
		...
		if(!activities.length){
			...
			return await new Promise(async(resolve)=>{
				...
				let currentScope = null,paramTags = $(activities[jsonCount].data).children('param'),matched_param_index =  null;
				if(paramTags.length){
					$(paramTags).each(function(index,item){
					  if($(paramTags).eq(index).attr('value') != undefined){
						  console.log(index,"   ",$(paramTags).eq(index).attr('value').indexOf('/config'));
						  if($(paramTags).eq(index).attr('value').indexOf('/config') > -1){
							  console.log("the current index is   ",index);
							  matched_param_index = index;
						  }
					  }
				  })
				}
				currentScope = $(activities[jsonCount].data).children('param').eq(matched_param_index).attr('value');
...
async function authUser(userInfo,logs,matchedData,widgetsCount){
	...
	/* customFiles1 = filesList.filter(function(el) {
		return el.indexOf('6e1_0750_ese_ee1_climatechange.html') == -1
	}); */
