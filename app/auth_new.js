...
let customFiles1 = ["671_7350_ese_ee3_usngthrmlengy.html"];
...
async function getJSON(json,counter,file){
    ...
    if(appManifesto.process == 'CPL'){
		...
		if(appManifesto.gradeLevel != 'kindergarten'){
			...
			//return;
			/* const audioUpdate = await writeJSON.write(audiosrcPath,renamedK2);
			console.log("audio json has been updated successfully   ",audioUpdate); */
	...
	let jsonValue = await new Promise(resolve => {
        ...
        if(appManifesto.process == 'CPL'){
			...
			const endPosjson = '"../../../../widget_data/config/'+json.jsonPath+'" /> <param name="theme" value="'+widgetSrc['value']+'" />';
            ...
            console.log(startPosjson_three,"   ----  ",endPosjson,"   ----    ",paramJSONpdateVal_three,"   paramJSONpdateVal");
            //return;
			...
			let paramHTMLUpdateVal = onlyHTML.htmlUpdate(startPoshtml,endPoshtml,checkoutDir+appManifesto.htmlPath);
		...
		if(customIndex < customFiles1.length){
			...
			if(customFiles1[customIndex] != undefined) {
				...
				if(matchedCpl.html && matchedCpl.html.length){
					if(appManifesto.process == 'CPL' && appManifesto.gradeLevel != 'kindergarten'){
						const startPoshtml = appManifesto.startDirectory;
						const endPoshtml = appManifesto.endDirectory+' data-version=""';
						let paramHTMLUpdateVal = onlyHTML.htmlUpdate(startPoshtml,endPoshtml,checkoutDir+appManifesto.htmlPath);
						console.log(startPoshtml,"   ----    ",endPoshtml,"    ----    ",paramHTMLUpdateVal,"   paramHTMLUpdateVal");
					} else if(appManifesto.process == 'NO-CPL'){
						...
						let paramHTMLval = onlyHTML.htmlUpdate(startPoshtml,endPosjsonhtml,checkoutDir+appManifesto.htmlPath);
...
async function authUser(userInfo,logs,matchedData,widgetsCount){
	...
	if(appManifesto.process == 'CPL' && widgetsCount == 0){
	...
	/* customFiles1 = filesList.filter(function(el) {
		return el.indexOf('681_7250_ese_ee2_fldfrcsactdstnc.html') == -1
	}); */
